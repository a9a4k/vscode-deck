...
**ImageDrop**:
An image dragged from outside VS Code onto a Terminal. Deck writes the dropped bytes to a file outside every Worktree and hands the pane that file's path as a bracketed paste; what the path *means* is the running agent's decision, not Deck's. Distinct from image **paste** (Cmd+V), where Deck forwards a keystroke and never sees the image — the pasteboard already holds it and a dropped file does not, which is the whole reason the two gestures work differently (ADR-0024, ADR-0054). Only images are claimed; anything else dropped still opens in an editor tab, because a copy of a source file would not be the file.
_Avoid_: attachment (the agent decides that — Deck only supplies a path), image paste (a different gesture with a different mechanism), upload (nothing leaves the machine), drop target (the Terminal is the thing, not the drop zone)
