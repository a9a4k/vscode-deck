...
**TerminalRemoval**:
Destroying a Terminal — killing its tmux session and removing its row. Surfaced as "Delete Terminal" in the Terminal row's right-click menu. Also happens when the shell `exit`s or when the Terminal's Worktree or Repository is removed. Closing the editor tab does **not** trigger it.
...
**TerminalOrder**:
The user-curated display order of a Worktree's rows — Terminals and Bookmarks interleaved, since a Bookmark is a sibling row and can sit between two Terminals. An order overlay that owns order but never existence: each key is resolved against the source that owns the row (the live tmux list for a Terminal, the Bookmark store for a Bookmark), and a key neither claims is dropped. Rows it does not name still appear, after the ones it does: uncurated Terminals by ascending `term-N`, then uncurated Bookmarks in the order they were pinned — so a newly pinned Bookmark lands at the bottom without the order being written at all. Creating a Terminal through Deck is an additive write that carries forward those rows and appends the new Terminal last; Terminals created outside Deck use the reader fallback. Only the reconciliation that observes live Terminals may drop a key; every other write adds or repositions one. Named for Terminals because they came first; it has always stored plain row keys, so admitting Bookmarks needed no migration.
...
- Opening a **Terminal** row was modeled on the Explorer opening a file: the tab appeared but the tree kept keyboard focus. Typing then went into the tree's type-navigation, hopping the highlight between rows and opening the wrong Terminal on Enter. Resolved: a **Terminal** is a typing surface, not a document — opening one focuses it, by every path (click, Enter, agent toast, Add Terminal). Restores ADR-0008 §7.
- Keyboard **TerminalRemoval** assumed `TreeView.selection` identified the row highlighted by arrow keys. It does not: VS Code tracks keyboard focus separately and its extension API exposes only selection ([microsoft/vscode#130880](https://github.com/microsoft/vscode/issues/130880)). Before Terminal opening took focus, Enter's selection commit plus focus-preserving open incidentally masked the gap; afterward a keyboard delete could target a different Terminal. Resolved: Delete Terminal is right-click only, because the context menu supplies the exact clicked row.
- Active **Terminal** tracking selected its tree row without moving keyboard focus, so opening Deck with `Cmd/Ctrl+Shift+.` left arrow-key navigation at the tree's old cursor. Resolved: that explicit shortcut opens Deck through its own command, then reveals the active Terminal with focus. Passive tab tracking stays selection-only so it never steals keyboard focus from a Terminal.
